...
var RadListView = (function (_super) {
    ...
    RadListView.prototype.updateSwipeActionsBehavior = function () {
        ...
        if (this.swipeActions === true) {
            if (!this._swipeActionsBehavior) {
                ...
                var that = new WeakRef(this);
                this._swipeActionsListener = new com.telerik.widget.list.SwipeActionsBehavior.SwipeActionsListener({
                    ...
                    onSwipeStarted: function (event) {
                        ...
                        var args = {
                            ...
                            swipeView: swipeView,
                            mainView: mainView,
                            itemIndex: event.swipedItemPosition(),
                        ...
                        if (listView.listViewLayout.scrollDirection === listViewCommonModule.ListViewScrollDirection.Horizontal) {
                            if (this.swipeLimits.top) {
                                that.get()._swipeActionsBehavior.setSwipeLimitStart(this.swipeLimits.top);
                            }
                            if (this.swipeLimits.bottom) {
                                that.get()._swipeActionsBehavior.setSwipeLimitEnd(this.swipeLimits.bottom);
                            }
                        }
                        else {
                            if (this.swipeLimits.right) {
                                that.get()._swipeActionsBehavior.setSwipeLimitStart(this.swipeLimits.right);
                            }
                            if (this.swipeLimits.left) {
                                that.get()._swipeActionsBehavior.setSwipeLimitEnd(this.swipeLimits.left);
                            }
                        }
                        if (this.swipeLimits.threshold) {
    ...
    RadListView.prototype.loadData = function () {
        if (!this.items || !this._android || !(this.itemTemplate || this.itemViewLoader)) {
            return;
        }
